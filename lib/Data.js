...
class Data{
    ...
    Stop() {
        util.GetLogger().debug('[Data.Stop] begin', this.__id);
        if (this.__IsStop()) {// 正常业务，不会触发多次stop；db异常有可能会
            return;
        }
        if (!this.__IsChanged() && !this.__IsCreated() && !this.__isSaving()) {
            util.GetLogger().info('[Data.Stop] key[%s] clear Data:%s', this.__key, this.__id);// 重要处理，打印一下
            GMgrImpl._mgr.RemoveData(this.__key, this.__id);
            return;
        }
        this.__SetStop(true);
        this.Save();
        util.GetLogger().debug('[Data.Stop] success', this.__id);
    }
    ...
    Save() {
        ...
        if (this.__lastErrorTime > Date.now()) {//错误后3秒内，不进行操作
            return;
        }
        //无改变则不处理
        //操作正在进行中,等操作结束，进行触发
        if ((!this.__IsChanged() && !this.__IsCreated()) || this.__isSaving()) {
            util.GetLogger().debug('[Data.Save] id[%s], !__IsChanged[%d] || !__IsCreated[%d] || __isSaving[%d]', this.__id, !this.__IsChanged(), !this.__IsCreated(), this.__isSaving());
            return;
        }
