...
for (let i = 0; i < 300; ++i) {
    ...
    tmp._id = mgo.IdString();
    G_objs.push(tmp);
}
// setInterval(()=>{
//     console.log(Object.keys(G_objs).length)
// }, 1000);reutrn;
...
const id = G_objs[G_objs.length-1]._id;//ObjectId().toString();
logger('id:%j', id);
let user = new Statistic(id);
const p = new Promise(function(re, rj) {
    logger('create start');
    setTimeout(() => {
        user.Create(G_objs[G_objs.length-1], (err) => {
            logger('create stop');
            if (err) {
                rj(err);
                return;
...
setTimeout(()=>{
    setInterval(()=>{
        ...
        let tmpids = ids.splice(0, len);
        logger(tmpids.length, ids.length);
        let t = setInterval(()=>{
            let tmp = tmpids.splice(0, 2000);
            if (tmpids.length == 0&&t) {
                clearInterval(t);
                t = undefined;
            }
